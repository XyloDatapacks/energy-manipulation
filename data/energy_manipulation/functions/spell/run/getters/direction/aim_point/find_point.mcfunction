...
data merge storage xylo_iris:settings {MaxRecursionDepth:100,TargetEntities:1b,Blacklist:"#xylo_iris:shape_groups/air",EntityHitboxTollerance:0.2}
data remove storage xylo_iris:settings Whitelist
$execute as $(uuid) at @s anchored eyes positioned ^ ^ ^ run function xylo_iris:get_target

execute store success score #xem.spell.run.getters.direction.aim_point.loop_start.result xem.op if data storage xylo_iris:output TargetPosition.pos
...
data modify storage energy_manipulation:op macro_input_position_in set from storage xylo_iris:output TargetPosition.pos
