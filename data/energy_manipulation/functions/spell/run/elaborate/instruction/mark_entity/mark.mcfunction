...
$execute if score xem.spell.run.getters.target.found xem.op matches 0 run data remove storage energy_manipulation:data players[{uuid:"$(caster)"}].marked_entities[{index:$(index)}]
