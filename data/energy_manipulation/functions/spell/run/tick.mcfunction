...
# grab data
data remove storage energy_manipulation:op shape_tick_data
data modify storage energy_manipulation:op shape_tick_data set from entity @s data.energy_manipulation

# check if caster died (out: "#xem.spell.run.caster_still_alive xem.op")
data remove storage energy_manipulation:op macro_data
data modify storage energy_manipulation:op macro_data.caster set from storage energy_manipulation:op shape_tick_data.spell_data.caster
...
execute if score #xem.spell.run.shape_tick xem.op matches 1 run function energy_manipulation:spell/run/shape/sort_tick with storage energy_manipulation:op shape_tick_data.shape
