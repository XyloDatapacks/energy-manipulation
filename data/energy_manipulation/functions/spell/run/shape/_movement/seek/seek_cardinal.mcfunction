...
$execute as $(target) at @s anchored $(anchor) positioned ^ ^ ^ positioned ~$(position_x) ~$(position_y) ~$(position_z) as $(uuid) facing entity @s feet positioned as @s positioned ^ ^ ^1 rotated $(yawn) $(pitch) positioned ^ ^ ^-7 facing entity @s feet on passengers run tp @s ~ ~ ~ ~ ~
